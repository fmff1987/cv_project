    ...
    public void onRowEdit(RowEditEvent event) {
        ...
        if (request.getRecruiter() != null && request.getRecruiter().getEmail().contains("@")) {
            if (request.getDeadline() != null) {
                ...
                String bodyMailWithDataFormat = "O candidato com o nome de " + request.getCandidateName() + 
                        " foi lhe atribuido a si até á Data Limite de " + dateFormat;
                this.sendMail(request.getRecruiter().getEmail(), bodyMailWithDataFormat);
                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(
                        FacesMessage.SEVERITY_INFO, "Notificação", "Email para recruta foi enviado."));
            } else {
    ...
    public void onRowEditOngoingList(RowEditEvent event) {
        ...
        if (request.getEstado() == Status.APROVADO) {
            if (request.getManager().getEmail().contains("@")) {
                ...
                FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(
                        FacesMessage.SEVERITY_INFO, "Notificação Email", "Email enviado para o Manager"));
            }
